// Обвести настоящий значок Ponoi в SVG. Запуск:
//   ../ponoi/node_modules/.bin/electron scripts/trace-mark.cjs
//
// ЗАЧЕМ. Знак в шапке сайта до сих пор был МОИМ рисунком «в духе» значка
// приложения — шестиугольник с кубиком внутри, нарисованный на глаз. Владелец
// прислал настоящий логотип, и рисовать похожее вместо него нельзя: логотип у
// продукта один, а «похоже» — это другой логотип.
//
// ПОЧЕМУ ОБВОДКА, А НЕ ПРОСТО PNG. Значок приложения — серый круг с белым
// кубом, и он рассчитан на тёмный фон: на белой странице светло-серый круг
// исчезает. Поэтому в шапку идёт САМ КУБ, отдельной фигурой и в currentColor —
// тогда он одинаково читается и на светлой теме, и на тёмной. Круглый значок
// (favicon, apple-touch-icon, картинка для соцсетей) остаётся настоящим PNG,
// как есть.
//
// КАК. Куб отделяется от круга по яркости (он почти белый, круг — серый),
// граница обводится по-пиксельно (алгоритм Мура), ломаная упрощается
// (Дуглас-Пекер) и переводится в координаты viewBox 64×64. Никаких кривых:
// в знаке одни прямые, и любая кривая означала бы ошибку обводки.
...
const ПОРОГ = 238          // ярче этого — куб; серый круг заметно темнее
...
app.whenReady().then(() => {
  ...
  // Маска куба: почти белый И непрозрачный.
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="64" height="64" role="img" aria-label="Ponoi">

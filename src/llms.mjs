import { ORIGIN, APP_URL, REPO_URL } from './site.mjs'

// llms.txt — краткая машиночитаемая выжимка о проекте.
//
// Правило, которого держимся: здесь не должно быть ни одного утверждения,
// которого нет на обычных страницах. Это указатель на них, а не второй,
...
Trackoteka: ${ORIGIN}/trackoteka/
FAQ: ${ORIGIN}/faq/
Transparency: ${ORIGIN}/transparency/
Press kit: ${ORIGIN}/press/
Source code: ${REPO_URL}
Current version: ${ctx.release.version}
...
- Category: messaging software / communication platform
...
Authoritative source: ${ORIGIN}/security/

Accurate summary: one-to-one direct messages can be end-to-end encrypted
(ECDH P-256, HKDF-SHA256, AES-256-GCM), together with their attachments and
calls. These are three separate settings and all are OFF by default. Group
conversations and server channels are NOT end-to-end encrypted. Call encryption
has not been verified in a real call and is labelled experimental. Attachments
sent without attachment encryption are stored in public cloud storage and can be
...
Authoritative source: ${ORIGIN}/privacy/
...
Authoritative source: ${ORIGIN}/transparency/

The project publishes its own list of weaknesses: attachments in public storage,
encryption off by default, no end-to-end encryption for groups and channels,
call encryption unverified in the field, no Content-Security-Policy, emoji images
loaded from a public CDN, a six-character minimum password, no two-factor
authentication, no external audit, and no open source licence yet.
...
the user before installing. Network access is restricted to declared domains
over https and wss only. There are no rate or resource limits on what an
installed plugin may do, which the app states before installation.

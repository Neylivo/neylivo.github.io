...
const t = {
  en: {
    title: 'Ponoi Plugins — Extend Your Messenger',
    desc: 'Ponoi plugins are single .ponoi JavaScript files that run sandboxed in a Web Worker with declared permissions. Add buttons, commands, panels, whole windows, bots and games.',
    h1: 'Make Ponoi yours.',
    lede: '<b>Extend Ponoi with plugins distributed as a single <code>.ponoi</code> file.</b> Write one in an afternoon, send it to a friend in a chat, and it installs from the message.',

    whyH: 'Why this exists',
    whyP: 'Most messengers let you pick a theme. Ponoi lets you change what the app does. If you want a button in the composer that rolls dice, a panel that shows your team’s deploy status, a slash command that talks to a language model, a game that lives in a window inside the app, or a bot that greets people in your server — you write it yourself instead of asking somebody to add it.',
    ...
    distP2: 'There is an editor inside the app, so you can write, run and fix a plugin without leaving Ponoi, and a workshop for plugins that need a 3D scene.',
    ...
    caps: [
      ...
      ['Windows and pages', 'Open a real window inside Ponoi with your own HTML: a full DOM with WebGL, WebGPU, WebAssembly, audio and unthrottled animation. Frameless and transparent windows are supported; the plugin can move them and knows when the user moves them.'],
  ...
  ru: {
    title: 'Плагины Ponoi — расширь свой мессенджер',
    desc: 'Плагины Ponoi (Поной) — это один файл .ponoi на JavaScript, который работает в песочнице Web Worker с объявленными разрешениями. Кнопки, команды, панели, целые окна, боты и игры.',
    h1: 'Сделай Ponoi своим.',
    lede: '<b>Расширяй Ponoi плагинами, которые раздаются одним файлом <code>.ponoi</code>.</b> Такой пишется за вечер, отправляется другу прямо в чат и ставится из сообщения.',

    whyH: 'Зачем это',
    whyP: 'В большинстве мессенджеров можно выбрать тему. В Ponoi можно поменять то, что приложение делает. Нужна кнопка в поле ввода, которая бросает кубик; панель со статусом выкладки у твоей команды; команда, которая разговаривает с языковой моделью; игра, живущая в окне внутри приложения; бот, который встречает людей на твоём сервере — ты пишешь это сам, а не просишь кого-то добавить.',
    ...
    distP2: 'Внутри приложения есть редактор — писать, запускать и чинить плагин можно не выходя из Ponoi, — и мастерская для плагинов, которым нужна 3D-сцена.',
    ...
    caps: [
      ...
      ['Окна и страницы', 'Открыть внутри Ponoi настоящее окно со своим HTML: полноценный DOM с WebGL, WebGPU, WebAssembly, звуком и анимацией без ограничения частоты. Есть окна без рамки и прозрачные; плагин может их двигать и знает, когда их двигает человек.'],
    ...
    sec3P: 'Домены плагин объявляет в шапке. Разрешены только <code>https:</code> и <code>wss:</code>, <code>Cookie</code> не разрешён никогда, а обратиться к самому Ponoi и его серверу плагин не может вовсе — он не сумеет обратить вашу сессию против приложения. Плагин вправе попросить доступ к <em>любым</em> сайтам, и тогда экран установки говорит об этом красным.',
...
function onLoad(ponoi) {
  ...
  ponoi.ui.addComposerButton({
    ...
    onClick: () =&gt; ponoi.messages.send('o/'),
  })

  ponoi.commands.register('hello', 'Say hello', args =&gt;
    ponoi.messages.send('Hello, ' + (args || 'world') + '!'))
}`

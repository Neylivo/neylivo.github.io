...
const t = {
  en: {
    title: 'What is Ponoi? — About Ponoi Messenger',
    desc: 'Ponoi (Поной) is a privacy-focused extensible messaging platform for private conversations, communities, calls, music and user-created plugins. Windows, Android and web.',
    h1: 'What is Ponoi?',
    lede: '<b>Ponoi (Поной) is a privacy-focused extensible messaging platform for private conversations, communities, calls, music and user-created plugins.</b>',

    p1: 'In plain terms: Ponoi is an app you talk to people in. You can write to someone directly, create a server with channels where a group of people gather, call each other with video and screen sharing, listen to music in the same room, and reshape the app itself — themes, fonts, layout, and plugins you or someone else wrote.',
    ...
    nameH: 'Ponoi and Поной',
    nameP: '<b>Ponoi</b> and <b>Поной</b> are the same product — the Latin and Cyrillic spellings of one name. The interface exists in both English and Russian. Searching for either name should lead here; this site is the official source of information about the project.',
    ...
    d6: '<b>Plugins.</b> A plugin is a single <code>.ponoi</code> file of JavaScript that runs sandboxed and can only do what its declared permissions allow. There is a built-in editor and a catalogue.',
    ...
    devH: 'Who makes Ponoi',
    devP: 'Ponoi is an independent project, not a company. It is developed in the open at <a href="' + REPO_URL + '" rel="noopener">github.com/ponoiai/ponoi</a>. There is no legal entity behind it, no funding round, and no investors — which is also why there is no code-signing certificate and no Google Play listing.',
    ...
    histH: 'Status',
    histP: 'Ponoi is under active development, and releases are frequent. Every version is listed with what changed in it on the <a href="/changelog/">changelog page</a>. Parts of the app that have not been verified in real use are marked as experimental rather than presented as finished — see <a href="/transparency/">transparency</a>.',

    linksH: 'Official links',
  },

  ru: {
    title: 'Что такое Поной — о мессенджере Ponoi',
    desc: 'Ponoi (Поной) — приватный расширяемый мессенджер для личного общения, сообществ, звонков, музыки и пользовательских плагинов. Windows, Android и веб.',
    h1: 'Что такое Поной?',
    lede: '<b>Ponoi (Поной) — приватный расширяемый мессенджер для личного общения, сообществ, звонков, музыки и пользовательских плагинов.</b>',

    p1: 'Если совсем просто: Ponoi — приложение, в котором общаются. Можно написать человеку напрямую, собрать сервер с каналами, где сидит компания, созвониться с видео и демонстрацией экрана, послушать музыку в одной комнате и переделать само приложение — темы, шрифты, расположение и плагины, свои или чужие.',
    ...
    nameH: 'Ponoi и Поной',
    nameP: '<b>Ponoi</b> и <b>Поной</b> — один и тот же продукт, написание латиницей и кириллицей. Интерфейс есть и на русском, и на английском. По любому из двух написаний человек должен попадать сюда: этот сайт — официальный источник сведений о проекте.',
    ...
    d6: '<b>Плагины.</b> Плагин — один файл <code>.ponoi</code> с кодом на JavaScript, который работает в песочнице и может только то, на что даны разрешения. Есть встроенный редактор и каталог.',
    ...
    devH: 'Кто делает Ponoi',
    devP: 'Ponoi — независимый проект, а не компания. Он разрабатывается открыто: <a href="' + REPO_URL + '" rel="noopener">github.com/ponoiai/ponoi</a>. За ним нет юридического лица, раунда инвестиций и инвесторов — отсюда же отсутствие сертификата подписи кода и страницы в Google Play.',
    ...
    histH: 'Состояние',
    histP: 'Ponoi активно разрабатывается, выпуски выходят часто. Каждая версия и то, что в ней изменилось, перечислены на <a href="/ru/changelog/">странице «Что нового»</a>. То, что не проверено в живой работе, помечено как экспериментальное, а не выдаётся за готовое, — см. <a href="/ru/transparency/">прозрачность</a>.',
...
export default {
  ...
  jsonld: (ctx, lang) => JSON.stringify({
    ...
    name: 'Ponoi',
    alternateName: ['Поной', 'Ponoi Messenger'],
    ...
    inLanguage: ['en', 'ru'],
    description: lang === 'en'
      ? 'Ponoi (Поной) is a privacy-focused extensible messaging platform for private conversations, communities, calls, music and user-created plugins.'
      : 'Ponoi (Поной) — приватный расширяемый мессенджер для личного общения, сообществ, звонков, музыки и пользовательских плагинов.',
  }),
...
<section class="band band-alt">
  <div class="wrap">
    ...
    <dl class="spec">
      <div><dt>${s.fName}</dt><dd>Ponoi</dd></div>
      <div><dt>${s.fAlt}</dt><dd>Поной</dd></div>

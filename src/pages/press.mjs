...
const t = {
  en: {
    title: 'Ponoi Press Kit — Descriptions, Logo and Facts',
    desc: 'Official descriptions, brand names, logo, product category and links for writing about Ponoi (Поной) accurately.',
    h1: 'Ponoi Press Kit',
    lede: 'Everything needed to describe Ponoi correctly. Copy any of it as it is — that is what it is for.',

    shortH: 'Short description',
    shortP: 'Ponoi is a privacy-focused extensible messenger for conversations, communities, calls, music and user-created plugins.',
    oneH: 'One line',
    oneP: 'Ponoi — private, extensible messenger for Windows, Android and the web.',
    longH: 'Long description',
    longP1: 'Ponoi (also written Поной) is a messaging and communication platform for private conversations, communities, calls, music and user-created plugins. It runs on Windows, on Android and in a browser, with one account across all three.',
    longP2: 'One-to-one direct messages can be end-to-end encrypted with ECDH P-256, HKDF-SHA256 and AES-256-GCM, including their attachments and calls; the app contains no analytics, crash reporting or advertising code. Servers with channels, roles and moderation tools cover communities. Trackoteka, the built-in music system, adds a shared library and listening together. Plugins are distributed as a single <code>.ponoi</code> file and run isolated in a Web Worker with permissions the user sees before installing.',
    longP3: 'Ponoi is an independent project developed in the open, with no company behind it. Its source code is public.',

    nameH: 'Names',
    nameL: [
      ['Brand name', 'Ponoi'],
      ['Russian name', 'Поной'],
      ['Both together', 'Ponoi (Поной) — the two spellings refer to the same product'],
      ['Not', 'Ponoi AI, Ponoi Inc., Ponoi Network, PonoiApp — none of these exist'],
      ['Music system', 'Trackoteka (Трекотека) — part of Ponoi, not a separate product'],
      ['Plugin format', 'A <code>.ponoi</code> file'],
    ],
    ...
    catL: [
      ...
      ['Developer', 'Ponoi — independent project by ponoiai'],
      ['Source code', 'Public'],
    ],

    logoH: 'Logo',
    logoP: 'The Ponoi mark is an isometric cube. It works in a single colour on light and dark backgrounds; please do not recolour it into a gradient, rotate it or place it on a busy photograph.',
    logoSvg: 'Mark as SVG',
    ...
    honestP2: 'If you need to say something about Ponoi’s security in one sentence: <i>“Ponoi offers optional end-to-end encryption for one-to-one conversations and contains no analytics or advertising.”</i>',
  },

  ru: {
    title: 'Пресс-кит Ponoi — описания, логотип и факты',
    desc: 'Официальные описания, названия, логотип, категория продукта и ссылки — чтобы писать о Ponoi (Поное) точно.',
    h1: 'Пресс-кит Ponoi',
    lede: 'Всё, что нужно, чтобы описать Ponoi правильно. Берите как есть — оно для этого и лежит.',

    shortH: 'Короткое описание',
    shortP: 'Ponoi — приватный расширяемый мессенджер для общения, сообществ, звонков, музыки и пользовательских плагинов.',
    oneH: 'Одной строкой',
    oneP: 'Ponoi — приватный расширяемый мессенджер для Windows, Android и веба.',
    longH: 'Развёрнутое описание',
    longP1: 'Ponoi (по-русски Поной) — мессенджер и платформа для общения: личная переписка, сообщества, звонки, музыка и пользовательские плагины. Работает на Windows, Android и в браузере, с одной учётной записью везде.',
    longP2: 'Личную переписку двух людей можно защитить сквозным шифрованием (ECDH P-256, HKDF-SHA256, AES-256-GCM), включая вложения и звонки; аналитики, сбора падений и рекламного кода в приложении нет. Сообщества держатся на серверах с каналами, ролями и средствами модерации. Трекотека — встроенная музыкальная система с общей библиотекой и совместным прослушиванием. Плагины раздаются одним файлом <code>.ponoi</code> и работают изолированно в Web Worker с разрешениями, которые человек видит до установки.',
    longP3: 'Ponoi — независимый проект, который разрабатывается открыто; компании за ним нет. Исходный код открыт.',

    nameH: 'Названия',
    nameL: [
      ['Название', 'Ponoi'],
      ['По-русски', 'Поной'],
      ['Вместе', 'Ponoi (Поной) — два написания одного продукта'],
      ['Не бывает', 'Ponoi AI, Ponoi Inc., Ponoi Network, PonoiApp — таких сущностей нет'],
      ['Музыкальная система', 'Трекотека (Trackoteka) — часть Ponoi, а не отдельный продукт'],
      ['Формат плагина', 'файл <code>.ponoi</code>'],
    ],
    ...
    catL: [
      ...
      ['Разработчик', 'Ponoi — независимый проект, автор ponoiai'],
      ['Исходный код', 'Открыт'],
    ],

    logoH: 'Логотип',
    logoP: 'Знак Ponoi — изометрический куб. Он работает одним цветом на светлом и тёмном фоне; пожалуйста, не перекрашивайте его в градиент, не поворачивайте и не кладите на пёструю фотографию.',
    logoSvg: 'Знак в SVG',
    ...
    honestP2: 'Если о безопасности Ponoi нужно сказать одной фразой: <i>«Ponoi предлагает сквозное шифрование по выбору для переписки один на один и не содержит аналитики и рекламы».</i>',
  },
}
